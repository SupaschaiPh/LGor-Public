...
public class Compiler extends SyntaxChecking implements Runnable {
    ...
    public void run() {
        ...
        int iii = 1;
        if (this.Err) {
            lb.setText("Error : " + this.CheckingErrorRes + ", Want to restart?");
            iii = JOptionPane.showConfirmDialog(null, lb, "Compiler", JOptionPane.OK_CANCEL_OPTION, JOptionPane.ERROR_MESSAGE);
        } else if (!this.charr.isAlive()) {
            lb.setText("You Die!!, Want to restart?");
            iii = JOptionPane.showConfirmDialog(null, lb, "Compiler", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, Theme.appIcon);

        } else if (!this.charr.isMeetCow()) {
            lb.setText("Don't Met Cow!!, Want to restart?");
            iii = JOptionPane.showConfirmDialog(null, lb, "Compiler", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, Theme.appIcon);

        } else if (this.playGroundCon.getPb().getCountMustKeepItem() > this.charr.getCountItem()) {
            lb.setText("Item not equal request, You got " + this.charr.getCountItem() + "/" + this.playGroundCon.getPb().getCountMustKeepItem() + ", Want to restart?");
            iii = JOptionPane.showConfirmDialog(null, lb, "Compiler", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, Theme.appIcon);

        } else if (this.charr.isMeetCow()) {
            lb.setText("Met Cow, You passed.");
            JOptionPane.showMessageDialog(null, lb, "Compiler", JOptionPane.PLAIN_MESSAGE, Theme.startIcon);
        }
